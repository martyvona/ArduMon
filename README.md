...
* The included host program `ardumon_client` can send canned command sequences to an Arduino running any ArduMon CLI.  Using a convenient scripting language, `ardumon_client` can also selectively either verify expected responses (e.g. to implement automated test scripts), ignore responses but delay between commands for flow control, and/or echo chosen responses for further use.
...
It is also acceptable to send data when a command handler is not actually running.  This can be leveraged to use one instance of ArduMon to invoke commands on another instance.  Or, for example, a received command could enable sending periodic status strings in text mode (or status packets in binary mode) indefinitely until another command is received to end the stream.  ArduMon can even be used for send-only applications, for example to broadcast a never ending stream of packets in binary mode.
